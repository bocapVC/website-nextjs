...
interface ConvocatoriaRowProps {
  convocatoria: Convocatoria;
}

/** Single accelerator application row: program + status badge, org, description, deadline, CTA. */
export function ConvocatoriaRow({ convocatoria }: ConvocatoriaRowProps) {
  return (
    <div className="flex flex-col gap-4 rounded-brand border border-line bg-surface-solid p-6 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <div className="flex flex-wrap items-center gap-2">
          <p className="font-semibold text-ink">{convocatoria.program}</p>
          <Badge tone={convocatoria.status === "vigente" ? "gold" : "neutral"}>
            {convocatoria.status === "vigente" ? "Vigente" : "Cerrada"}
          </Badge>
        </div>
        <p className="mt-1 text-xs uppercase tracking-wide text-ink-soft">
          {convocatoria.organization}
        </p>
        <p className="mt-2 text-sm text-ink-soft">{convocatoria.description}</p>
      ...
      {convocatoria.status === "vigente" ? (
        convocatoria.url ? (
          <Button href={convocatoria.url} external size="sm" className="shrink-0">
            Postular
          </Button>
        ) : (
